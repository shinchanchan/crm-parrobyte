...
import { seedCreditConfigs, getUserCredits, getAllCreditConfigs, getVisibleServices, checkCredits, chargeCredits, deductCredits } from "./server/lib/credits.js";
import { processAllUsers as processYouTubeComments } from "./server/lib/youtube.js";
import { processAllUsers as processInstagramComments } from "./server/lib/instagram.js";
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'","https://parrobyte.redmooncrackers.co.in"],
      styleSrc: ["'self'", "'unsafe-inline'", "https://cdn.tailwindcss.com", "https://cdn.jsdelivr.net", "https://fonts.googleapis.com","https://parrobyte.redmooncrackers.co.in"],
      scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https://cdn.tailwindcss.com", "https://parrobyte.redmooncrackers.co.in", "https://cdn.jsdelivr.net", "https://*.razorpay.com"],
      scriptSrcAttr: ["'unsafe-inline'"],
      fontSrc: ["'self'", "https://fonts.gstatic.com", "https://cdn.jsdelivr.net","https://parrobyte.redmooncrackers.co.in"],
      imgSrc: ["'self'", "data:", "blob:", "https:", "http:"],
      connectSrc: ["'self'", "https://*.razorpay.com", "wss://*.razorpay.com","https://parrobyte.redmooncrackers.co.in"],
      frameSrc: ["'self'", "https://*.razorpay.com","https://parrobyte.redmooncrackers.co.in"],
      frameAncestors: ["'self'", "https://*.razorpay.com","https://parrobyte.redmooncrackers.co.in"],
    },
  },
  referrerPolicy: {
...
app.use(fileUpload({
  ...
  abortOnLimit: true,
  responseOnLimit: "File size limit exceeded. Max allowed is 100MB.",
}));
...
app.use(async (req, res, next) => {
  ...
  // ===== CREDITS: inject credit balance and visible services =====
  if (req.session.user) {
    try {
...
app.use("/forms", requireAuth, formRoutes);
...
app.get("/logout", async (req, res) => {
  if (req.session.user) {
    // Clear session token from DB (single-login cleanup)
    try {
...
// ===== Scheduled messages cron job (NON-BLOCKING) =====
// Uses setImmediate to prevent event loop blocking and node-cron warnings
cron.schedule("*/1 * * * *", async () => {
  // Offload to next tick to prevent blocking the cron scheduler
  setImmediate(async () => {
    try {
      const now = new Date();
      ...
      console.log(`[Cron] Processing ${scheduled.length} scheduled message(s)`);

      for (const msg of scheduled) {
        // Mark as processing immediately to prevent duplicate sends from next cron tick
        ...
        let allSent = true;
        try {
          const contactIds = JSON.parse(msg.contactIds);
          if (!Array.isArray(contactIds) || contactIds.length === 0) {
            console.log(`[Cron] Scheduled msg ${msg.id} has no contacts`);
            continue;
          }

          // Fetch only the specific contacts (efficient query)
          ...
          if (!targetContacts.length) {
            console.log(`[Cron] Scheduled msg ${msg.id}: no matching contacts found`);
            continue;
          }
          ...
          for (let ci = 0; ci < targetContacts.length; ci++) {
            ...
            try {
              await waManager.sendMessage(msg.sessionId, contact.phone, msg.content, msg.type, msg.mediaUrl, msg.interactiveData ? JSON.parse(msg.interactiveData) : null);
              sentCount++;
              console.log(`[Cron] Scheduled msg ${msg.id} sent to ${contact.phone}`);
            } catch (sendErr) {
              console.error(`[Cron] Failed to send scheduled msg ${msg.id} to ${contact.phone}:`, sendErr.message);
              allSent = false;
            }
            // 30-second gap between messages to avoid WhatsApp rate limits
          ...
          if (sentCount > 0) {
            try {
              const creditService = msg.type === "poll" ? "poll_message" : "schedule_message";
              await deductCredits(msg.userId, creditService, sentCount, `Scheduled msg ${msg.id}: ${sentCount} sent`);
              console.log(`[Cron] Deducted ${sentCount} credits for scheduled msg ${msg.id}`);
            } catch (creditErr) {
              console.error(`[Cron] Credit deduction failed for scheduled msg ${msg.id}:`, creditErr.message);
            }
          }
        } catch (err) {
          console.error("[Cron] Scheduled message error:", err.message);
          allSent = false;
        } finally {
          // Always update status so it doesn't get picked up again
          await db.update(schema.scheduledMessages)
            .set({
              status: allSent ? "completed" : "failed",
              completedAt: allSent ? new Date() : null,
            })
            .where(eq(schema.scheduledMessages.id, msg.id));
        }
      }
