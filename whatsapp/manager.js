...
import { isSystemOverloaded, getSystemResources } from "../server/lib/queue.js";
import os from "os";
...
function findChromeExecutable() {
  if (process.env.PUPPETEER_EXECUTABLE_PATH) {
    // Resolve symlinks to avoid Puppeteer issues with symlink chains
    try {
      const realPath = realpathSync(process.env.PUPPETEER_EXECUTABLE_PATH);
      return realPath;
    } catch (e) {
...
class WhatsAppManager {
  ...
  async createSession(userId, sessionName, sessionId = null, isRestore = false) {
    ...
    const client = new Client({
      ...
      puppeteer: {
        ...
        args: [
          ...(this.getMaxSessions() >= 25 ? ["--single-process"] : []),
          "--no-sandbox",
    ...
    client.on("message_create", async (msg) => {
      ...
      // Process incoming message inline (non-blocking)
      setImmediate(() => {
        this.handleIncomingMessage(session.id, msg).catch(e => console.error("[MessageHandler] Inline error:", e.message));
      });
    });
    ...
    if (lastError) {
      ...
      // During restore, DON'T mark as disconnected — let restoreAllSessions handle retry logic
      // This prevents transient errors (RAM limit, browser lock) from permanently killing sessions
      if (!isRestore) {
        this.sessionStatus.set(clientId, "disconnected");
        try {
          await db.update(schema.whatsappSessions)
        ...
        console.log(`[${clientId}] Restore attempt failed — NOT marking disconnected, will retry later`);
      }
    ...
    try {
      ...
      for (let i = 0; i < uniqueSessions.length; i++) {
        ...
        try {
          ...
          await this.createSession(session.userId, session.sessionName, session.id, true);
          restored++;
        } catch (err) {
          ...
          // Check if it's a real auth corruption error
          // Target.setAutoAttach often means the page crashes during init due to bad session data
          const isAuthCorrupted =
            (errMsg.includes("auth") || errMsg.includes("Auth") || errMsg.includes("LOGOUT") || errMsg.includes("Target.setAutoAttach")) &&
            !errMsg.includes("already running") &&
            !errMsg.includes("Max") &&
            !errMsg.includes("memory");

          if (isAuthCorrupted) {
            // ONLY clear auth if it's truly corrupted
            console.log(`[Restore] Auth corrupted for session ${session.id} — clearing auth data`);
            await this._cleanupAuth(clientId);
            try {
              await db.update(schema.whatsappSessions)
            ...
            // Transient error (RAM limit, browser lock, etc.) — keep status as connecting
            // so next restart or auto-reconnect can try again
            console.log(`[Restore] Transient error for session ${session.id} — keeping auth, will retry on next startup`);
            // Leave status unchanged (connecting/connected) so restoreAllSessions will try again
          }
        }
      }
      ...
      // ── Auto-retry failed restores after 30 seconds ──
      if (failed > 0 && restored > 0) {
        console.log(`[Restore] Scheduling retry for ${failed} failed session(s) in 30s...`);
        setTimeout(() => {
          this.restoreAllSessions().catch(e => console.error("[Restore] Retry error:", e.message));
        }, 30000);
      }
    } catch (err) {
  ...
  async reconnectSession(sessionId) {
    ...
    return await this.createSession(s.userId, s.sessionName, sessionId, true);
  }
}
