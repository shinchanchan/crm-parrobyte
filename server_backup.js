...
app.get("/instagram/webhook", (req, res) => {
  ...
  console.log("[Instagram Webhook] GET verification attempt:");
  console.log("  mode:", mode);
  console.log("  token:", token);
  console.log("  challenge:", challenge ? "present" : "missing");
  console.log("  expectedToken:", expectedToken ? "set" : "NOT SET IN ENV");

  if (!expectedToken) {
    console.error("[Instagram Webhook] INSTAGRAM_WEBHOOK_VERIFY_TOKEN is not set in .env");
  ...
  console.log("[Instagram Webhook] Verification failed. mode:", mode, "tokenMatch:", token === expectedToken);
  res.sendStatus(403);
});

app.post("/instagram/webhook", express.json(), async (req, res) => {
  try {
    console.log("[Instagram Webhook] POST received. Object:", req.body?.object, "entries:", req.body?.entry?.length || 0);
...
app.get("/sitemap.xml", async (req, res) => {
  try {
    const baseUrl = "https://crm.parrobyte.co.in";
    ...
    // Dynamic public enquiry forms
    const forms = await db.select({ slug: schema.enquiryForms.slug, updatedAt: schema.enquiryForms.updatedAt })
      .from(schema.enquiryForms)
      .where(eq(schema.enquiryForms.isActive, true));

    const formUrls = forms.map(f => ({
      loc: `/forms/p/${f.slug}`,
      priority: "0.7",
      changefreq: "monthly",
      lastmod: f.updatedAt ? new Date(f.updatedAt).toISOString().split("T")[0] : today,
    }));

    const allUrls = [...staticUrls, ...formUrls];
