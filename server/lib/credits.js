...
export async function getUserActiveSubscription(userId) {
  try {
    ...
    const subs = await db.select()
      ...
      .where(
        and(
          ...
          gte(schema.userSubscriptions.expiresAt, now)
        )
      )
      .orderBy(sql`${schema.userSubscriptions.expiresAt} desc`)
      .limit(1);
    ...
    const plans = await db.select()
      ...
      .where(eq(schema.subscriptionPlans.id, subs[0].planId));

    if (!plans.length) return null;

    return { subscription: subs[0], plan: plans[0] };
  } catch (e) {
    console.error("[Credits] getUserActiveSubscription error:", e.message);
    return null;
  }
}

/** Check if a service is covered by user's active subscription */
export async function isServiceCoveredBySubscription(userId, serviceKey) {
  const activeSub = await getUserActiveSubscription(userId);
  if (!activeSub) return false;

  try {
    const included = JSON.parse(activeSub.plan.includedServices || "[]");
    // "*" means all services included
    if (included.includes("*")) return true;
    return included.includes(serviceKey);
  } catch (e) {
    console.error("[Credits] Failed to parse includedServices:", e.message);
    return false;
  }
}
...
/** Check if user has enough credits (does NOT deduct). Respects free quota and active subscriptions. */
export async function checkCredits(userId, serviceKey, quantity = 1) {
  ...
  // Check active subscription first — subscription covers this service = unlimited usage
  const subCovered = await isServiceCoveredBySubscription(userId, serviceKey);
  if (subCovered) {
    creditLog(`checkCredits: subscription covers ${serviceKey} — unlimited usage`);
    const balance = await getUserCredits(userId);
    return { allowed: true, balance, cost: 0, isFree: false, isSubscription: true };
  }
  ...
  if (balance < totalCost) {
    ...
    return {
      ...
      balance,
      cost: totalCost,
      message: `Insufficient credits. This action requires ₹${totalCost} credits. You have ₹${balance} credits. Please top up or subscribe to a monthly plan.`,
    };
  }
...
/** Deduct credits from user and log transaction. Respects free quota and active subscriptions.
...
export async function deductCredits(userId, serviceKey, quantity = 1, description = "", referenceId = null) {
  ...
  // Check active subscription first — subscription covers this service = no deduction
  const subCovered = await isServiceCoveredBySubscription(userId, serviceKey);
  if (subCovered) {
    creditLog(`deductCredits: subscription covers ${serviceKey} — skipping deduction`);
    const current = await getUserCredits(userId);
    // Log as subscription usage (0 cost, marked as subscription)
    await db.insert(schema.creditTransactions).values({
      userId,
      type: "subscription",
      amount: 0,
      ...
      serviceKey,
      description: description ? `(SUBSCRIPTION) ${description}` : `(SUBSCRIPTION) ${serviceKey} x${quantity}`,
      referenceId,
    });
    return { success: true, deducted: 0, balance: current, isSubscription: true };
  }
  ...
  if (!result.success) {
    ...
    return { success: false, error: `Insufficient credits. Need ₹${totalCost}, have ₹${balance}. Subscribe to a monthly plan for unlimited usage.`, balance };
  }
