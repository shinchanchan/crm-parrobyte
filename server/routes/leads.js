...
// In-memory progress tracker for lead sends (single-process only)
const leadSendJobs = new Map(); // jobId -> { userId, type, total, sent, failed, status, startedAt, completedAt }
let leadJobIdCounter = 1;

function getLeadJobsForUser(userId) {
  return Array.from(leadSendJobs.values()).filter(j => j.userId === userId);
}

function cleanupOldLeadJobs() {
  const now = Date.now();
  for (const [id, job] of leadSendJobs.entries()) {
    if (job.completedAt && (now - job.completedAt) > 3600000) { // 1 hour
      leadSendJobs.delete(id);
    }
  }
}
setInterval(cleanupOldLeadJobs, 600000); // Clean every 10 minutes
...
router.get("/", async (req, res) => {
  try {
    ...
    res.render("pages/leads/index", {
      ...
      isAdmin,
      leadJobs: getLeadJobsForUser(userId),
    });
  } catch (error) {
...
// Send WhatsApp to leads (non-blocking background processing)
router.post("/send-whatsapp", async (req, res) => {
  try {
        ...
        if (msgType === 'poll' && templateRows[0].variables) {
          try {
            ...
            if (vars.pollOptions && vars.pollOptions.length >= 2) {
              interactiveData = { options: vars.pollOptions, allowMultipleAnswers: vars.allowMultipleAnswers === true };
            }
          } catch (e) {}
        }
      }
    ...
    const leadsData = await db.select().from(schema.leads).where(eq(schema.leads.userId, userId));
    ...
    const jobId = leadJobIdCounter++;
    const job = {
      id: jobId, userId, type: "whatsapp", total: targetLeads.length,
      sent: 0, failed: 0, status: "running", startedAt: Date.now(),
    };
    leadSendJobs.set(jobId, job);
    req.flash("success", `WhatsApp sending started for ${targetLeads.length} lead(s). Progress will update on the page.`);
    res.redirect("/leads");

    // Process in background
    const waManager = req.waManager;
    setImmediate(async () => {
      ...
      for (let i = 0; i < targetLeads.length; i++) {
        ...
        try {
          await waManager.sendMessage(sessionId, lead.phone, personalizedMessage, msgType, media, interactiveData);
          await db.update(schema.leads).set({ lastContactedAt: new Date() }).where(eq(schema.leads.id, lead.id));
          job.sent++;
        } catch (err) {
          console.error(`[Leads WhatsApp] ${lead.phone} failed:`, err.message);
          job.failed++;
        }
        if (i < targetLeads.length - 1) {
          const randomGap = Math.floor(Math.random() * 20000) + 30000; // 30000-50000ms
          console.log(`[WhatsApp Leads] Waiting ${(randomGap / 1000).toFixed(1)}s before next message...`);
      ...
      if (job.sent > 0) {
        try {
          await deductCredits(userId, creditService, job.sent, `Sent WhatsApp to ${job.sent} lead(s)`);
        } catch (e) {
          console.error("[Leads WhatsApp] Credit deduction failed:", e.message);
        }
      }
      console.log(`[Leads WhatsApp] Job ${jobId} completed: ${job.sent} sent, ${job.failed} failed`);
    });

  } catch (error) {
    console.error("Send WhatsApp to leads error:", error);
    req.flash("error", "Failed to start WhatsApp sending: " + error.message);
    res.redirect("/leads");
  }
});

// Send Email to leads (non-blocking background processing)
router.post("/send-email", async (req, res) => {
  try {
    ...
    const jobId = leadJobIdCounter++;
    const job = {
      id: jobId, userId, type: "email", total: targetLeads.length,
      sent: 0, failed: 0, status: "running", startedAt: Date.now(),
    };
    leadSendJobs.set(jobId, job);
    req.flash("success", `Email sending started for ${targetLeads.length} lead(s). Progress will update on the page.`);
    res.redirect("/leads");
    ...
    setImmediate(async () => {
      ...
      for (let i = 0; i < targetLeads.length; i++) {
        ...
        try {
          ...
          await db.insert(schema.emailMessages).values({
            userId, templateId: templateId ? parseInt(templateId) : null,
            direction: "outbound", fromEmail: config.fromEmail || config.emailUser,
            toEmail: lead.email, subject: filledSubject, body: filledBody,
            attachments: attachments.length ? JSON.stringify(attachments.map(a => ({ filename: a.filename, size: a.content?.length || 0 }))) : null,
            status: "sent", contactId: lead.id, sentAt: new Date(),
          });
          ...
          console.error(`[Leads Email] ${lead.email} failed:`, err.message);
          job.failed++;
        }
        if (i < targetLeads.length - 1) {
      ...
      if (job.sent > 0) {
        try {
          await deductCredits(userId, "send_email", job.sent, `Email to ${job.sent} lead(s)`);
        } catch (e) {
          console.error("[Leads Email] Credit deduction failed:", e.message);
        }
      }
      console.log(`[Leads Email] Job ${jobId} completed: ${job.sent} sent, ${job.failed} failed`);
    });

  } catch (error) {
    console.error("Send email to leads error:", error);
    req.flash("error", "Failed to start email sending: " + error.message);
    res.redirect("/leads");
  }
});

// Get lead send progress
router.get("/send-status", async (req, res) => {
  try {
    const userId = req.session.user.id;
    const jobs = getLeadJobsForUser(userId);
    res.json({ active: jobs.length > 0, jobs });
  } catch (error) {
    res.json({ active: false, jobs: [], error: error.message });
  }
});
