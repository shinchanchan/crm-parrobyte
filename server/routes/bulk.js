      ...
      for (let i = 0; i < contactData.length; i++) {
        ...
        try {
          ...
          if (i < contactData.length - 1) {
            ...
            console.log(`[Bulk] Waiting ${(randomGap / 1000).toFixed(1)}s before next message (contact ${i + 2}/${contactData.length})...`);
...
// Get bulk job status - returns ALL active jobs for the user from DB
router.get("/status", async (req, res) => {
  try {
    ...
    const jobs = await db.select().from(schema.bulkMessageJobs)
      .where(
        isAdmin
          ? eq(schema.bulkMessageJobs.status, "processing")
          : and(
              eq(schema.bulkMessageJobs.status, "processing"),
              eq(schema.bulkMessageJobs.userId, userId)
            )
      );

    if (!jobs.length) {
      return res.json({ active: false, jobs: [] });
    }

    const jobList = jobs.map(job => ({
      id: job.id,
      total: job.totalContacts || 0,
      sent: job.sentCount || 0,
      failed: job.failedCount || 0,
      status: job.status,
      progress: Math.round(((job.sentCount || 0 + job.failedCount || 0) / (job.totalContacts || 1)) * 100),
    }));

    res.json({ active: true, jobs: jobList });
  } catch (error) {
    res.json({ active: false, error: error.message, jobs: [] });
  }
});
