...
router.post("/run", async (req, res) => {
  try {
    ...
    // ── RUN SCRAPER INLINE (non-blocking) ──
    setImmediate(async () => {
      try {
        await runScrapeJob(userId, job.id, query, location, max);
        console.log(`[Scraper] Job ${job.id} completed inline`);
      } catch (err) {
        ...
        await db.update(schema.scrapingJobs)
          ...
          .where(eq(schema.scrapingJobs.id, job.id));
      }
    });
  } catch (error) {
