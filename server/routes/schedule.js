...
router.post("/update/:id", async (req, res) => {
  try {
    ...
    if (!schedule.length || (schedule[0].userId !== userId && !isAdmin)) {
      req.flash("error", "Unauthorized");
      return res.redirect("/schedule");
    }

    // If editing a completed/failed/cancelled job, reset to pending so it runs again
    const shouldResetStatus = schedule[0].status !== "pending" && schedule[0].status !== "processing";
    ...
    let finalContent = content;
    let mediaUrl = schedule[0].mediaUrl;
    let msgType = type || schedule[0].type;
    let interactiveData = schedule[0].interactiveData;
    ...
    req.flash("success", shouldResetStatus ? "Scheduled message updated and reset to pending" : "Scheduled message updated");
    res.redirect("/schedule");
  } catch (error) {
    console.error("Update schedule error:", error);
    req.flash("error", "Failed to update scheduled message");
    res.redirect("/schedule");
  }
});
...
router.post("/delete/:id", async (req, res) => {
  try {
    ...
    req.flash("success", "Scheduled message deleted");
    res.redirect("/schedule");
  } catch (error) {
    console.error("Delete schedule error:", error);
    req.flash("error", "Failed to delete scheduled message");
    res.redirect("/schedule");
  }
});
