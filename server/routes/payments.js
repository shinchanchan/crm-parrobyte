...
import { getActiveSubscriptionPlans } from "../lib/credits.js";
...
router.post("/create-subscription-order", async (req, res) => {
  try {
    ...
    const planRows = await db.select().from(schema.subscriptionPlans)
    ...
    const invoiceResult = await db.insert(schema.invoices).values({
      ...
      status: "pending",
      notes: `Subscribe to ${plan.displayName} (${plan.periodDays} days)`,
    }).returning();
    const invoice = invoiceResult[0];
    ...
    res.json({
      ...
      notes: `Subscribe to ${plan.displayName}`,
    });
  } catch (error) {
    console.error("[Payments] Create subscription order error:", error.message);
    res.status(500).json({ success: false, error: error.message });
  }
});
 ...
 * Body: { razorpay_order_id, razorpay_payment_id, razorpay_signature, invoiceId }
 * Verifies payment and activates user subscription
 */
router.post("/verify-subscription", async (req, res) => {
  try {
    ...
    const planRows = await db.select().from(schema.subscriptionPlans)
      .where(eq(schema.subscriptionPlans.name, invoice.planName));
    if (!planRows.length) {
      return res.status(404).json({ success: false, error: "Subscription plan not found" });
    }
    const plan = planRows[0];
    ...
    await db.update(schema.invoices)
      .set({
        ...
        paymentMethod: "razorpay",
        paidAt: new Date(),
        notes: `Paid via Razorpay. Subscription: ${plan.displayName}`,
      })
      .where(eq(schema.invoices.id, invoice.id));

    // Calculate subscription period
    const now = new Date();
    const expiresAt = new Date(now.getTime() + plan.periodDays * 24 * 60 * 60 * 1000);

    // Create user subscription
    const subResult = await db.insert(schema.userSubscriptions).values({
      userId,
      planId: plan.id,
      status: "active",
      startedAt: now,
    ...
    console.log(`[Payments] Subscription activated: user=${userId}, plan=${plan.displayName}, expires=${expiresAt.toISOString()}`);
    ...
    try {
      ...
      const invoiceData = {
        ...
        userName: user?.name || "Customer",
        userEmail: user?.email || "",
        description: `Subscription: ${plan.displayName} (${plan.periodDays} days)`,
      ...
      const pdfBuffer = await generateInvoicePdf(invoiceData);
      await sendInvoiceEmail(user?.email, user?.name, invoiceData, pdfBuffer);
      console.log(`[Payments] Subscription invoice PDF emailed for invoice ${invoice.id}`);
    } catch (pdfErr) {
      console.error("[Payments] Invoice PDF/email failed:", pdfErr.message);
    }
    ...
    res.json({
      success: true,
      message: `Subscription activated! ${plan.displayName} is now active until ${expiresAt.toLocaleDateString("en-IN")}.`,
      subscription: {
    ...
    if (event.event === "payment.captured") {
      ...
      if (payment && payment.notes?.invoiceId) {
        ...
        if (type === "subscription") {
          ...
          const planId = parseInt(payment.notes.planId);
          const planRows = await db.select().from(schema.subscriptionPlans)
            .where(eq(schema.subscriptionPlans.id, planId));
          if (planRows.length) {
            const plan = planRows[0];
