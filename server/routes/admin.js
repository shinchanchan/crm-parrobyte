...
import { getAllCreditConfigs, getAllCreditConfigsAdmin, addCredits, getActiveSubscriptionPlans } from "../lib/credits.js";
...
router.get("/subscription-plans", async (req, res) => {
  try {
    ...
    const configs = await getAllCreditConfigsAdmin();
    res.render("pages/admin/subscriptionPlans", {
      title: "Subscription Plans - ParroByte CRM",
      plans,
      configs,
    });
  } catch (error) {
...
router.post("/subscription-plans/create", async (req, res) => {
  try {
    const { name, displayName, description, price, periodDays, includedServices, maxSessions, maxContacts, maxTemplates, sortOrder } = req.body;
    const services = Array.isArray(includedServices) ? includedServices : (includedServices ? [includedServices] : []);
    await db.insert(schema.subscriptionPlans).values({
      ...
      maxContacts: parseInt(maxContacts) || 1000,
      maxTemplates: parseInt(maxTemplates) || 50,
      sortOrder: parseInt(sortOrder) || 0,
    });
    req.flash("success", "Subscription plan created");
    res.redirect("/admin/subscription-plans");
  } catch (error) {
    console.error("Create subscription plan error:", error);
    req.flash("error", "Failed to create subscription plan: " + error.message);
    res.redirect("/admin/subscription-plans");
  }
});

router.post("/subscription-plans/update/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { displayName, description, price, periodDays, includedServices, maxSessions, maxContacts, maxTemplates, isActive, sortOrder } = req.body;
    const services = Array.isArray(includedServices) ? includedServices : (includedServices ? [includedServices] : []);
    ...
    req.flash("success", "Subscription plan updated");
    res.redirect("/admin/subscription-plans");
  } catch (error) {
...
router.post("/subscription-plans/delete/:id", async (req, res) => {
  try {
    const { id } = req.params;
...
router.get("/user-subscriptions", async (req, res) => {
  try {
    ...
    res.render("pages/admin/userSubscriptions", {
      title: "User Subscriptions - ParroByte CRM",
      subscriptions: subs,
      userMap,
      planMap,
    });
    ...
    req.flash("error", "Failed to load user subscriptions");
    res.redirect("/admin");
  }
});
